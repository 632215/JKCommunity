...
//                LogUtils.i("日志上传失败" + s);
    ...
    @Override
    public void getUpdateUserPasswordDirectly(String account, String pass, final IStartPageListener listener) {
        ServerUnit.getInstance().updateUserPasswordDirectly(account, pass, null, new ServerUnit.OnServerUnitListener() {
            @Override
            public void success(List list, String s) {
                LogUtils.i("修改用户密码成功!");
                listener.onRegisterFail("密码修改成功");
            }

            @Override
            public void failure(String s, int i) {
                LogUtils.i("修改用户密码失败!");
                listener.onRegisterFail("用户密码修改成功");
            }
        });
