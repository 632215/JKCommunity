...
//                        }
        ...
        ServerUnit.getInstance().login(account, pwd, null, new ServerUnit.OnServerUnitListener() {
            ...
            public void failure(String s, int i) {
                ...
                if (s.equals("当前用户不存在")) {
                    ...
                    listener.onInputPassWord(account, pwd);
    ...
    @Override
    public void getUpdateUserPasswordDirectly(String account, String pass, final IScanningLockCodListener listener) {
        ServerUnit.getInstance().updateUserPasswordDirectly(account, pass, null, new ServerUnit.OnServerUnitListener() {
            @Override
            public void success(List list, String s) {
                LogUtils.i("修改用户密码成功" + s);
                listener.onRegisterSuccess("密码修改成功!");
            }

            @Override
            public void failure(String s, int i) {
                LogUtils.i("修改用户密码失败" + s);
                listener.onRegisterSuccess("密码修改失败!");
            }
        });
    }
}
